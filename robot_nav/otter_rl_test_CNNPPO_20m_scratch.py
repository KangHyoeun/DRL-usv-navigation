...
def main(args=None):
    ...
    max_steps = 150  # maximum number of steps in single episode
    test_scenarios = 100
    ...
    model = CNNPPO(
        ...
        device=device,
        load_model=True,
        model_name="otter_CNNPPO_20m_scratch_BEST",
        load_directory=Path("robot_nav/models/PPO/best_checkpoint"),
    )  # instantiate a model

    sim = OtterSIM(
        world_file="/worlds/imazu_scenario/s1.yaml", disable_plotting=True, enable_phase1=False, max_steps=max_steps
